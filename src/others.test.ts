...
for (const env of envs) {
    ...
    describe(env.serverWallet.chain.name, () => {
        ...
        it('eth_call transfer with insufficient funds', async () => {
        ...
        it('eth_estimate with insufficient funds', async () => {
            try {
        ...
        it('eth_estimate call caller with insufficient funds', async () => {
            try {
                await env.emptyWallet.estimateContractGas({
                    address: await getErrorTesterAddr(),
                    abi: ErrorsAbi,
        ...
        it('eth_estimate with revert', async () => {
            try {
        ...
        it('eth_get_balance with no account', async () => {
        ...
        it('eth_estimate with insufficient funds to cover gas', async () => {
            const balance = await env.serverWallet.getBalance(
                env.emptyWallet.account,
            )
            expect(balance).toBe(0n)
        ...
        it('eth_estimate with no gas specified', async () => {
            const balance = await env.serverWallet.getBalance(
                env.emptyWallet.account,
            )
            expect(balance).toBe(0n)
